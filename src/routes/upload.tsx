...
import { SECTORS, PROVINCES, type Sector, type Province } from "@/lib/data";
...
function UploadPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const id = String(projects.length + 1 + Date.now());
    addProject({
      id,
      title: title.trim(),
      sector: sector as Sector,
      province: province as Province,
      investmentUsdM: parseFloat(capex) || 0,
      description: description.trim() || "Submitted via VELA developer portal.",
      developer: "Your Organization",
      docs: docs as Record<CriteriaKey, boolean> & { offtake: boolean; permits: boolean; financial: boolean; esg: boolean; sponsor: boolean; strategic: boolean },
      score,
    });
    toast.success("Project submitted!", { description: `Bankability score: ${score}/100` });
      ...
      <div className="mx-auto max-w-4xl px-6 py-16">
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          <button
            ...
            className="group flex flex-col gap-5 rounded-xl border border-border bg-card shadow-xs p-8 text-left hover:border-primary/40 hover:shadow-sm transition-all"
          >
            <div className="w-12 h-12 rounded-xl border border-border bg-secondary group-hover:bg-primary/10 flex items-center justify-center transition-colors">
          ...
          <button
            ...
            className="group relative flex flex-col gap-5 rounded-xl border border-primary/25 bg-card shadow-xs p-8 text-left hover:border-primary/50 hover:shadow-sm transition-all"
          >
            <div className="absolute top-4 right-4 text-[10px] font-semibold text-accent bg-accent/10 border border-accent/20 px-2 py-0.5 rounded-full">
              Recommended
            </div>
            <div className="w-12 h-12 rounded-xl vela-gradient flex items-center justify-center shadow-xs shadow-primary/20">
            ...
            <span className="text-xs vela-gradient-text font-medium group-hover:underline mt-auto">
              Let AI do the work →
            </span>
          </button>
        </div>
    ...
    <div className="mx-auto max-w-7xl px-6 py-10">
      <div className="flex items-center gap-3 mb-8 flex-wrap">
        ...
        <button
          ...
          className="ml-auto text-xs text-muted-foreground hover:text-foreground border border-border rounded-full px-3 py-1 transition-colors"
        >
          Switch mode
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid md:grid-cols-12 gap-6">
          <div className="md:col-span-7 space-y-5">
            ...
            <div className={cn(
              "rounded-xl border border-border bg-card shadow-xs p-6 space-y-5 transition-all duration-300",
              aiFilledInfo && "ring-1 ring-primary/20",
            )}>
            ...
            {mode === "hybrid" && (
              <div className="rounded-xl border border-border bg-card shadow-xs p-6 space-y-4">
                      ...
                      <button
                        ...
                        className={cn(
                          "w-full flex items-center gap-3 rounded-xl border p-4 text-left transition-all",
                            ...
                            ? "border-success/30 bg-success/6 cursor-default"
                            : "border-border bg-secondary hover:border-primary/40 hover:bg-primary/4 cursor-pointer",
                        )}
                      >
                        {done
            ...
            {mode === "ai-first" && (
              <div className="rounded-xl border border-border bg-card shadow-xs p-5">
                <p className="text-xs font-semibold text-muted-foreground mb-3">Criterion Coverage</p>
                  ...
                  {CRITERIA_KEYS.map(key => (
                    <div
                      ...
                      className={cn(
                        ...
                        docs[key] ? "bg-success/10 text-success" : "bg-secondary text-muted-foreground",
                      )}
                    >
                      {docs[key]
          ...
          <div className="md:col-span-5">
            <div className="sticky top-24 rounded-xl border border-border bg-card shadow-sm p-6 space-y-5">
              <h2 className="text-sm font-semibold">Live Bankability Score</h2>
              ...
              {topGap && (
                <div className="rounded-lg border border-accent/25 bg-accent/6 px-4 py-3">
                  <p className="text-[11px] text-accent font-semibold mb-1">Highest impact next</p>
