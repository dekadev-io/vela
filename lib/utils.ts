...
import type { BankabilityTier, CriteriaKey, ScoreCriteria } from './types'
...
export const CRITERIA_DESC: Record<CriteriaKey, string> = {
  offtake:   'Long-term buyer commitment',
  permits:   'Regulatory readiness',
  financial: 'Returns and capital structure',
  esg:       'Environmental & social safeguards',
  sponsor:   'Track record and credibility',
  strategic: 'National priority fit',
}
...
export function computeScoreFromDocs(docs: Record<CriteriaKey, boolean>): number {
  ...
  return (Object.keys(docs) as CriteriaKey[])
    .filter((k) => docs[k])
...
export function getTierMeta(score: number): TierMeta {
  if (score >= 85) return {
    label: 'Investment Ready', key: 'investment-ready',
    color: 'text-success', bg: 'bg-success/15', ring: 'ring-success/40',
    nextThreshold: null, nextLabel: null,
  }
  if (score >= 70) return {
    label: 'Near Bankable', key: 'near-bankable',
    color: 'text-accent', bg: 'bg-accent/15', ring: 'ring-accent/40',
    nextThreshold: 85, nextLabel: 'Investment Ready',
  }
  if (score >= 50) return {
    label: 'Development', key: 'development',
    color: 'text-amber', bg: 'bg-amber/15', ring: 'ring-amber/40',
    nextThreshold: 70, nextLabel: 'Near Bankable',
  }
  return {
    label: 'Early Stage', key: 'early-stage',
    color: 'text-danger', bg: 'bg-danger/15', ring: 'ring-danger/40',
    nextThreshold: 70, nextLabel: 'Near Bankable',
  }
}
